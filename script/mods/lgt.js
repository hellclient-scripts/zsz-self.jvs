...
function on_lgt(name, line, wildcards)
{
	...
	switch (name) {
		...
		case "lgt_later":   //^(本日闯关你已经试过了，明天中午|你再等等吧，现在)
			...
			switch (l_door) {
				case 0:
					set("lgt/cmd","unride;wu");
					break;				
				case 1: 
					set("lgt/cmd","unride;nu");
					break;
				case 2:
					set("lgt/cmd","unride;eu");
					break;				
				case 3:
					set("lgt/cmd","unride;su");
					break;
				case 4:
					set("lgt/cmd","unride;u");
...
*/
		...
		case "lgt_heal1" : //^你(现在气血充盈，不需要疗伤。|运功完毕，吐出一口瘀血，脸色看起来好多了。)
			...
			if (wcs[0].indexOf("运功完毕") != -1)
				send("mtc");
			send(get_var("lgt_cmd_pre"));
			break;

		case "lgt_up" ://"^(你听到无数天魔在耳边吟唱嘶吼，莫大的压力使你开始神智迷糊了|你现在气血充盈，不需要疗伤|一个空洞的声音在你耳边响起：你在磨蹭什么)"
			send("halt;" + get_var("lgt_cmd_pre")+";summon "+get_var("id_weapon"));
